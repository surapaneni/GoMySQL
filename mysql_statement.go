...
func (stmt *MySQLStatement) Prepare(sql string) (err os.Error) {
	mysql := stmt.mysql
	if mysql.Logging {
		log.Printf("Prepare statement called")
	}
	...
	if mysql.Logging {
		log.Printf("[" + strconv.Uitoa(uint(mysql.sequence-1)) + "] Sent prepare command to server")
	}
	// Get result packet(s)
	for {
		// Get result packet
...
func (stmt *MySQLStatement) SendLongData(num uint16, data string) (err os.Error) {
	mysql := stmt.mysql
	if mysql.Logging {
		log.Printf("Send long data called")
	}
	// Check statement has been prepared
	if !stmt.prepared {
		stmt.error(CR_NO_PREPARE_STMT, CR_NO_PREPARE_STMT_STR)
	...
	mysql.sequence++
	if mysql.Logging {
		log.Printf("[" + strconv.Uitoa(uint(mysql.sequence-1)) + "] " + "Sent long data packet to server")
	}
	return
}
...
func (stmt *MySQLStatement) Execute() (res *MySQLResult, err os.Error) {
	mysql := stmt.mysql
	if mysql.Logging {
		log.Printf("Execute statement called")
	}
	// Check statement has been prepared
	if !stmt.prepared {
		stmt.error(CR_NO_PREPARE_STMT, CR_NO_PREPARE_STMT_STR)
	...
	mysql.sequence++
	if mysql.Logging {
		log.Printf("[" + strconv.Uitoa(uint(mysql.sequence-1)) + "] " + "Sent execute statement to server")
	}
	// Get result packet(s)
	for {
		// Get result packet
...
func (stmt *MySQLStatement) Close() (err os.Error) {
	mysql := stmt.mysql
	if mysql.Logging {
		log.Printf("Close statement called")
	}
	// Lock mutex and defer unlock
	mysql.mutex.Lock()
	defer mysql.mutex.Unlock()
	...
	if mysql.Logging {
		log.Printf("[" + strconv.Uitoa(uint(mysql.sequence-1)) + "] Sent close statement command to server")
	}
	return
}
...
func (stmt *MySQLStatement) Reset() (err os.Error) {
	mysql := stmt.mysql
	if mysql.Logging {
		log.Printf("Reset statement called")
	}
	// Lock mutex and defer unlock
	mysql.mutex.Lock()
	defer mysql.mutex.Unlock()
	...
	if mysql.Logging {
		log.Printf("[" + strconv.Uitoa(uint(mysql.sequence-1)) + "] Sent reset statement command to server")
	}
	return
}
...
func (stmt *MySQLStatement) getPrepareResult() (err os.Error) {
	...
	default:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received unknown packet from server with first byte: " + fmt.Sprint(c))
		}
	...
	case c == ResultPacketOK:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received ok for prepared statement packet from server")
		}
		// Save statement info
	...
	case c == ResultPacketError:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received error packet from server")
		}
		// Return error response
	...
	case c >= 0x01 && c <= 0xfa && stmt.ParamCount > 0 && !stmt.paramsEOF:
		...
		stmt.paramsRead++
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received param packet from server (ignored)")
		}
	// Field packet
	case c >= 0x01 && c <= 0xfa && stmt.result.FieldCount > 0 && !stmt.result.fieldsEOF:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received field packet from server")
		}
	// EOF Packet fe
	case c == ResultPacketEOF:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received eof packet from server")
		}
		// Change EOF flag
		if stmt.ParamCount > 0 && !stmt.paramsEOF {
			stmt.paramsEOF = true
			if mysql.Logging {
				log.Printf("End of param packets")
			}
		} else if !stmt.result.fieldsEOF {
			stmt.result.fieldsEOF = true
			if mysql.Logging {
				log.Printf("End of field packets")
			}
		}
	}
	// Increment sequence
...
func (stmt *MySQLStatement) getExecuteResult() (err os.Error) {
	...
	case c == ResultPacketOK && !stmt.resExecuted:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received ok packet from server")
		}
		// Create result
		stmt.result = new(MySQLResult)
	...
	case c >= 0x01 && c <= 0xfa && !stmt.resExecuted:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received result set packet from server")
		}
	...
	case c == ResultPacketOK && stmt.resExecuted:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received binary row data packet from server")
		}
	// EOF Packet fe
	case c == ResultPacketEOF:
		pkt := new(packetEOF)
