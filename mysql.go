...
func (mysql *MySQL) Close() (err os.Error) {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence-1) + "] " + "Sent quit command to server")
	}
	// Close connection
	mysql.conn.Close()
	mysql.connected = false
	if mysql.Logging {
		log.Printf("Closed connection to server")
	}
	return
}
...
func (mysql *MySQL) Query(sql string) (res *MySQLResult, err os.Error) {
	if mysql.Logging {
		if len(sql) > 512 {
			trim := sql[0:512]
			log.Printf("Query called with SQL: " + trim + "...")
		} else {
			log.Printf("Query called with SQL: " + sql)
		}
	}
	// If not connected return
	if !mysql.connected {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence-1) + "] " + "Sent query command to server")
	}
	// Get result packet(s)
	for {
		// Get result packet
...
func (mysql *MySQL) MultiQuery(sql string) (res []*MySQLResult, err os.Error) {
	if mysql.Logging {
		if len(sql) > 512 {
			trim := sql[0:512]
			log.Printf("MultiQuery called with SQL: " + trim + "...")
		} else {
			log.Printf("MultiQuery called with SQL: " + sql)
		}
	}
	// If not connected return
	if !mysql.connected {
...
func (mysql *MySQL) ChangeDb(dbname string) (err os.Error) {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence-1) + "] " + "Sent change db command to server")
	}
	// Get result packet
	err = mysql.getResult()
	return
...
func (mysql *MySQL) Ping() (err os.Error) {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence-1) + "] " + "Sent ping command to server")
	}
	// Get result packet
	err = mysql.getResult()
	return
...
func (mysql *MySQL) InitStmt() (stmt *MySQLStatement, err os.Error) {
	if mysql.Logging {
		log.Printf("Initialise statement called")
	}
	// If not connected return
	if !mysql.connected {
		err = os.NewError("A connection to a MySQL server is required to use this function")
...
func (mysql *MySQL) connect() (err os.Error) {
	...
	if mysql.auth.host == "localhost" || mysql.auth.host == "127.0.0.1" {
		...
		if mysql.Logging {
			log.Printf("Connected using unix socket")
		}
		// Connect via TCP
	} else {
		...
		if mysql.Logging {
			log.Printf("Connected using TCP/IP")
		}
	}
...
func (mysql *MySQL) init() (err os.Error) {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received init packet from server")
	}
...
func (mysql *MySQL) authenticate() (err os.Error) {
	...
	if mysql.Logging {
		log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Sent auth packet to server")
	}
	// Increment sequence
	mysql.sequence++
	return
...
func (mysql *MySQL) getResult() (err os.Error) {
	...
	default:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received unknown packet from server")
		}
	...
	case c >= 0x01 && c <= 0xfb && !mysql.curRes.rowsEOF:
		...
		if mysql.Logging {
			log.Printf("[" + fmt.Sprint(mysql.sequence) + "] Received row data packet from server")
		}
	// EOF Packet fe
	case c == ResultPacketEOF:
		...
		if !mysql.curRes.fieldsEOF {
			...
			if mysql.Logging {
				log.Printf("End of row data packets")
			}
			mysql.addResult()
		}
	}
...
func (mysql *MySQL) addResult() {
	...
	if mysql.Logging {
		log.Printf("Current result set saved")
	}
	// Reset temp result
	mysql.curRes = nil
